...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        {
            if ( accessGrid[locs.get( x ).getRow()][locs.get( x )
    ...
    {
        ArrayList<Location> listLoc = getPossibleMoves( loc );
        for ( int x = 0; x < listLoc.size(); x++ )
        {
            accessGrid[listLoc.get( x ).getRow()][listLoc.get( x ).getCol()]--;
        }
