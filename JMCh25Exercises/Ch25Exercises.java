...
public class Ch25Exercises
{
    ...
    {
        completeTree[0] = (Integer)completeTree[0]
            - ( ( (Integer)completeTree[0] + 1 ) / 2 );
    }
    ...
    public Object[] toArray( TreeNode root, int numNodes )
    {
        Object[] obj = new Object[numNodes + 1];
        obj[0] = numNodes;
        toArray( root, obj, 1 );
        return obj;
    }
    ...
    public boolean isHeap( Integer[] items, int numNodes )
    {
        ...
        while ( index <= numNodes )
        {
            if ( ( 2 * index < items.length
                && items[index] > items[2 * index] )
                || ( 2 * index + 1 < items.length
    ...
    {
        int x = 1;
        while ( x * 2 + 1 <= numNodes + 1 && items[x * 2] < items[x]
            && items[x * 2 + 1] < items[x] )
        {
            if ( items[x * 2] < items[x * 2 + 1] )
            {
                Integer temp = items[x];
                items[x] = items[x * 2];
                items[x * 2] = temp;
                x = x * 2;
            }

            else if ( items[x * 2 + 1] < items[x * 2] )
            {
                Integer temp = items[x];
                items[x] = items[x * 2 + 1];
                items[x * 2 + 1] = temp;
    ...
    {
        PriorityQueue<Message> msg = new PriorityQueue<Message>();
