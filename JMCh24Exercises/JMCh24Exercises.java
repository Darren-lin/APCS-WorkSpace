...
public class JMCh24Exercises
{
    ...
    public boolean has3OnSameDay( Date[] birthdays )
    {
        ...
        for ( Date date : birthdays )
        {
            ...
            {
                map.replace( date, map.get( date ) + 1 );
                if ( map.get( date ) >= 3 )
                {
                    return true;
                }
    ...
    public String mostFrequentWord( Scanner input )
    {
        ...
        {
            strings = input.nextLine().toLowerCase().split( "\\W+" );
        ...
        do
        {
            ...
            if ( response.length() > 0 )
            {
                ...
                switch ( response.charAt( 0 ) )
                {
                    case '1':
                        @SuppressWarnings("deprecation")
                        Date[] d = { new Date( 2010, 7, 4 ),
                            new Date( 2010, 1, 1 ), new Date( 2010, 7, 4 ),
                            new Date( 2010, 12, 1 ), new Date( 2010, 7, 1 ),
                            new Date( 2010, 7, 4 ) };
                        System.out.println(
                            "3 on same date = " + ex.has3OnSameDay( d ) );
                        break;
                    case '2':
