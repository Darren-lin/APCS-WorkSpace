...
/**
 * This class deletes black squares that are touching each other (not including
 * corners). Basically deletes big objects in a grid.
 *
 * @author Darren Lin
 * @version 11/14/19
 * @author Period: 4
...
public class EraseObject extends World<Tile>
{
    ...
    private void erase( Grid<Tile> gr, int row, int col )
    {
        ...
        if ( !gr.isValid( location ) || gr.get( location ) == null
            || gr.get( location ).getColor() == ERASE_COLOR )
        {
            return;
        }
        else
