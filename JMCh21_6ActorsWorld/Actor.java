...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        while ( !mailbox.isEmpty() )
        {
            ...
            Message messageSent = mailbox.remove();

            String message = messageSent.getText();
            if ( message.contains( "have" ) )
            {
                String hasItem = messageSent.getText().substring( 5 );

                if ( countPossessions( hasItem ) == 0 )
                {
                    send( messageSent.getSender(), "ship " + hasItem );
                }
            }
            else if ( message.contains( "need" ) )
            {
                ...
                if ( countPossessions( hasItem ) > 1 )
                {
                    send( messageSent.getSender(), "have " + hasItem );
                }
            }
            else if ( message.contains( "ship" ) )
            {
                ...
                if ( countPossessions( hasItem ) > 1 )
                {
                    if ( messageSent.getSender()
                        .receiveItem( messageSent.getRecipient(),
                            hasItem ) == true )
                    {
                        myPossessions.remove( hasItem );
                    }
                }
            }
        }
